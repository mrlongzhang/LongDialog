...
from bot.corpus.cornelldata import CornellData
from bot.corpus.opensubsdata import OpensubsData
from bot.corpus.scotusdata import ScotusData
from bot.corpus.ubuntudata import UbuntuData
from bot.corpus.lightweightdata import LightweightData
...
class Batch:
    ...
    def __init__(self):
        self.encoderSeqs = []
        self.encoderSeqsLength =[]
        self.decoderSeqs = []
        self.targetSeqs = []
        self.targetSeqsLength=[]
        self.weights = []
...
class TextData:
    ...
    def _createBatch(self, samples, transpose = False):
        ...
        for i in range(batchSize):
            ...
            batch.decoderSeqs.append(sample[1])  # New seq2seq needs no <go> and <eos> tokens
            batch.targetSeqs.append(batch.decoderSeqs[-1][0:])  # Same as decoder, but shifted to the left (ignore the <go>)
        ...
        #    print(self.sequence2str(sample[0]))
        #    print(self.sequence2str(sample[1]))  # Check we did not modified the original sample
    ...
    def loadCorpus(self):
        ...
        if not datasetExist:  # First time we load the database: creating all files
            ...
            if self.args.vocabularySize > len(self.word2id): # a greater vocabulary size is not necessary
                self.args.vocabularySize = len(self.word2id)
    ...
    def printBatch(self, batch, trans = False):
        ...
        print('----- Print batch -----')
        if not trans:
            for i in range(len(batch.encoderSeqs)):
                print('Encoder: {}'.format(self.sequence2str(batch.encoderSeqs[i])))
                print('Decoder: {}'.format(self.sequence2str(batch.decoderSeqs[i])))
                print('Targets: {}'.format(self.sequence2str(batch.targetSeqs[i])))
                print('Weights: {}'.format(self.sequence2str(batch.weights[i])))
        else:
    ...
    def decodyn2setence(self, predicts, beam_size = 1):
        """
        Decode output into a (list of) sentence 
        We return text directly
        """
        sentence = []
        for predict in predicts:    
            for i in range(beam_size):
                predict_list = np.ndarray.tolist(predict[:,:,i])
                predict_seq = [self.id2word[idx] for idx in predict_list[0]]
                sentence.append(' '.join(predict_seq))
    ...
    def deco2sentence(self, decoderOutputs):
        ...
        sentence = self.sequence2str(sequence, clean=True)
