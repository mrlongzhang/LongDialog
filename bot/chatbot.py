...
from bot.model import DynamicRNNS2SModel as Model
from bot.seq2seqbasic import Model as OldModel
...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        globalArgs.add_argument('--debug', action='store_true', help='run LongDialog with Tensorflow debug mode. Read TF documentation for more details on this.')
        ...
        nnArgs.add_argument('--embeddingSize', type=int, default=1024, help='embedding size of the word representation')
        ...
        nnArgs.add_argument('--beamSearch', action='store_true', help='if pesent, the program will use beam search in decoder')
        nnArgs.add_argument('--beamSize', type=int, default=5, help='beam size if beam search is enabled for decoder')
        nnArgs.add_argument('--maxGradientNorm', type=float, default=4., help='maximum gradient norm for graident clip.')


        # Training options
        trainingArgs = parser.add_argument_group('Training options')
        trainingArgs.add_argument('--numEpochs', type=int, default=20, help='maximum number of epochs to run')
        ...
        trainingArgs.add_argument('--learningRate', type=float, default=0.0002, help='Learning rate')
    ...
    def main(self, args=None):
        ...
        """
        print('Welcome to LongDialog v0.1 !')
        print()
        print('Running with Tensorflow: v{}'.format(tf.__version__))
        ...
        try:  # If the user exit while training, we still try to save the model
            for e in range(self.args.numEpochs):
                ...
                for nextBatch in tqdm(batches, desc="Training"):
                    print('Debug batch content')
                    #self.textData.printBatch(batches[0])
                        
                    ops, feedDict = self.model.step(nextBatch)
                    assert len(ops) == 3  # training, loss
                    _, loss, summary = sess.run(ops, feedDict)
    ...
    def mainTestInteractive(self, sess):
        ...
        print('')
        print('Welcome to the interactive mode of LongDialog. Type \'exit\' or just press ENTER to quit the program. Have fun.')
    ...
    def singlePredict(self, question, questionSeq=None):
        ...
        if self.args.beamSearch:
            answer = self.textData.decodyn2setence(output, self.args.beamSize)
        else:
            answer = self.textData.decodyn2setence(output, 1)
    ...
    def _saveSession(self, sess):
        ...
        with open(model_name, 'w') as f:  # HACK: Simulate the old model existance to avoid rewriting the file parser
            f.write('This file is used internally by LongDialog to check the model existance. Please do not remove.\n')
    ...
    def loadModelParams(self):
        ...
        if not self.args.reset and not self.args.createDataset and os.path.exists(configName):
            ...
            self.args.beamSearch = config['Network'].getboolean('beamSearch')
            self.args.beamSize = config['Network'].getint('beamSize')
            self.args.maxGradientNorm= config['Network'].getfloat('maxGradientNorm')
            ...
            print('beamSearch: {}'.format(self.args.beamSearch))
            print('beamSize: {}'.format(self.args.beamSize))
            print('maxGradientNorm: {}'.format(self.args.maxGradientNorm))
    ...
    def saveModelParams(self):
        ...
        config['Network']['beamSearch'] = str(self.args.beamSearch)
        config['Network']['beamSize'] = str(self.args.beamSize)
        config['Network']['maxGradientNorm'] = str(self.args.maxGradientNorm)
